...
% (a) geometric-mean absolute circularity
% (b) geometric-mean relative circularity
...
function [lambda_GA, lambda_GR] = lambda_GA_GR(Gamma)

[Cycles, ArcsInCycles, M] = find_cycles(Gamma);

% Geometric-mean absolute circularity, i.e. lambda_GA:
numerator = 0;
remainingArcs = 1:length(M.Edges{:,1});
sum_flowsNotInCycles = 0;

for k = 1:size(Cycles,1) % compute the numerator
   cycle_k_GM = cycle_geometric_mean(ArcsInCycles{k}, M);
   numerator = numerator + cycle_k_GM; 
end
...
lambda_GA = numerator/denominator;

% Geometric-mean relative circularity:
lambda_GR = numerator;
